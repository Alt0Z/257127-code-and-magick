...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillRect(TEXT_X + (BAR_WIDTH + BAR_GAP) * i, BAR_Y, BAR_WIDTH, (-BAR_MAX_HEIGHT * times[i]) / maxTime);
  }

  for (var j = 0; j < names.length; j++) {
    ctx.fillStyle = 'rgba(0, 0, 0, 1)';
    ctx.fillText(Math.floor(times[j]), TEXT_X + (BAR_WIDTH + BAR_GAP) * j, CLOUD_Y + 1.5 * BAR_GAP);
